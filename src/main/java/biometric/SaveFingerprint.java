...
public class SaveFingerprint extends JPanel implements ActionListener {
	...
	private static Fmd finalFmd;

	private SaveFingerprint(Reader reader, boolean bStreaming) {
		m_reader = reader;
	...
	private void saveToDB(User user) {		
		...
		bio.setUser_id(user.getId());
		bio.setBiometric(finalFmd.getData());
		bioRepo().save(bio);
	}

	public static void Run(Reader reader, boolean bStreaming, User user) {
		JDialog dlg = new JDialog((JDialog) null, "Put your finger on the reader", true);
		SaveFingerprint capture = new SaveFingerprint(reader, bStreaming);		
		capture.doModal(dlg);
		capture.saveToDB(user);
	}
}
